...
tmux send-keys -t session1:0.0 "docker exec -it oai-ext-dn iperf -s -i 1 -B 192.168.70.135" C-m

# Send the second command to the second pane --> transmit data from the UE containers
tmux send-keys -t session1:0.1 "docker exec -it tt-nrue1 bash" C-m
...
tmux send-keys -t session1:0.1 "iperf -t 86400 -i 1 -fk -c 192.168.70.135 -b 20M -B $net_address" C-m
...
# tmux send-keys -t session1:0.3 "docker exec -it oai-ext-dn iperf -u -t 86400 -i 1 -fk -B 192.168.70.135 -b 20M -c $net_address" C-m
