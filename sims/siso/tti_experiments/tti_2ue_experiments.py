...
def autoUE():
    ...
    for ktap in range(start_tap,end_tap+1,4):
        for kk in range(start_ue,end_ue+1,3):
            ...
            time.sleep(2)
            for jk in range(161,161+kk): 
                os.system(f"docker exec -d oai-ext-dn iperf -u -t 86400 -i 1 -fk -B 192.168.70.135 -b 50M -c 10.0.0.{jk-159}")
                os.system(f"docker exec -d tt-ue{jk-160} iperf -t 86400 -i 1 -fk -c 192.168.70.135 -b 50M -B 10.0.0.{jk-159} -p 52{ik-149}")
            time.sleep(5)
    ...
    for ktap in range(start_tap,end_tap+1,4):
        ...
        for kk in range(start_ue,end_ue+1,3):
            ...
            for rnti in all_vals.keys():
                ...
                ax8.plot(all_vals[rnti]['dl_mcs_ttis'], all_vals[rnti]['dl_mcs_values'], label=f'{rnti} DL MCS')
            ...
            for rnti in all_vals.keys():
                ax3.plot(all_vals[rnti]['ul_tpt_ttis'], all_vals[rnti]['ul_tpt_values'], label=f'{rnti} UL TPT')
